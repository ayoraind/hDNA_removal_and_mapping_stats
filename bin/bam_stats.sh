...
mapped_reads=$(samtools stats ${FILE}.alignment.sorted.bam |  grep ^SN | awk 'NR == 7 {print $4}')
unmapped_reads=$(samtools stats ${FILE}.alignment.sorted.bam |  grep ^SN | awk 'NR == 9 {print $4}')
total=$(samtools stats ${FILE}.alignment.sorted.bam |  grep ^SN | awk 'NR == 1 {print $5}')
